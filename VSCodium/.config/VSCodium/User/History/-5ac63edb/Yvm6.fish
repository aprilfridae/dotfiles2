...
function ocean
    ...
    # 4. Corrected Matugen Syntax
    # Flags go AFTER 'image' in newer versions
    # Use --mode and --scheme instead of -m and -s
    # The -- separates the sub-command from the flags to prevent the 'unexpected' error
    matugen image "$wall_path" -- --mode dark --scheme scheme-neutral
